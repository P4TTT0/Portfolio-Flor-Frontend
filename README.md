...
- Node.js 18+
- A Sanity project (free tier works) with the schema above
...
│   ├── globals.css                # tokens, textures, animations
│   ├── layout.tsx                 # fonts, metadata
│   ├── page.tsx                   # entry → <LandingContent />
│   └── LandingContent.tsx         # <main> + section composition
│
├── components/
│   ├── bio/BioCard.tsx
│   ├── contact/ContactForm.tsx
│   ├── craft/                     # Clip, Tape (decorative primitives)
│   ├── demos/
│   │   ├── DemoCard.tsx
│   │   └── VideoPopup.tsx         # YouTube modal (notebook-styled)
│   ├── landing/
│   │   ├── Header.tsx
│   │   └── RansomName.tsx         # ransom-style name renderer
│   ├── samples/
│   │   ├── FolderPapers.tsx       # SVG-path-based paper sheets
│   │   ├── TrackList.tsx
│   │   ├── TrackItem.tsx
│   │   └── WaveformPlayer.tsx     # SVG ink-stroke waveform
│   ├── sections/                  # Bio, Demos, Samples, Works, Social, Contact, SectionNav
│   └── social/SocialCard.tsx
│
├── hooks/
│   ├── useAudioPlayer.ts          # Web Audio API + waveform peaks
│   └── useBreakpoint.ts
│
└── lib/
    ├── landing-data.ts            # static fallback (name, role, bio)
    ├── platform-config.ts         # social media icons + colors
    ├── sanity.ts                  # Sanity client
    ├── use-sanity-content.ts      # profile / demos / samples / social
    └── youtube-utils.ts           # extractYouTubeId, getYouTubeThumbnail, getYouTubeEmbedUrl

public/
└── assets/
    ├── animated/                  # PNG titles (demos, muestras, redes, trabajos)
    ├── elements/                  # paper, pin, clip, pizarra, polaroid, contact
    ├── letters/                   # ransom-name PNG variants
    └── textures/                  # kraft, rough-paper, cartoon
```
